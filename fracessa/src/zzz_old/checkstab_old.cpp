...
void fracessa::check_stability()
{
    ...
    RationalMatrixSmall bee(extended_support_size_reduced, extended_support_size_reduced);
    ...
    std::vector< RationalMatrixSmall> bee_vee(r+1);
    ...
    for (size_t v=1; v<=r; v++) {
        ...
        bee_vee[v] = RationalMatrixSmall(kay_vee_size[v], kay_vee_size[v]);
        ...
        small_rational pivot = bee_vee[v-1](pivot_pos, pivot_pos);
        if (pivot <= small_rational(0)) {
            if (conf_with_log_ && logger_)
        ...
        RationalMatrixSmall rank1(kay_vee_size[v-1], kay_vee_size[v-1]);
