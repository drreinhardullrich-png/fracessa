...
template<typename T>
void fracessa::check_stability(const rational_linalg::Matrix<T>& game_matrix, rational_linalg::Matrix<T>& Bee)
{
    bitset64 bitsetm = candidate_.support.lowest_set_bit(); //get lowest set bit as bitfield
    bitset64 extended_support_reduced = candidate_.extended_support.subtract(bitsetm); //ext support without m
    size_t m = candidate_.support.find_first();
    size_t extended_support_size_reduced = candidate_.extended_support_size - 1;

    if (conf_with_log_ && logger_) {
        logger_->info("Support: {}", candidate_.support.to_bitstring(dimension_));
        logger_->info("Support size: {}", candidate_.support.count());
        logger_->info("Extended support: {}", candidate_.extended_support.to_bitstring(dimension_));
    ...
    bitset64 kay = candidate_.extended_support.subtract(candidate_.support); //extended_support without support
...
template void fracessa::check_stability<rational>(const rational_linalg::Matrix<rational>&, rational_linalg::Matrix<rational>&);
template void fracessa::check_stability<small_rational>(const rational_linalg::Matrix<small_rational>&, rational_linalg::Matrix<small_rational>&);
